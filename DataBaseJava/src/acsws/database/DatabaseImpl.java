...
import acsws.database.pojos.Image;
...
public class DatabaseImpl implements DataBaseOperations, ComponentLifecycle {
	...
	public int storeProposal(Target[] targets) {
		HashSet<acsws.database.pojos.Target> targetsP =  new HashSet<acsws.database.pojos.Target>();
		for (Target t: targets) {
			acsws.database.pojos.Target tp = new acsws.database.pojos.Target();
			tp.setExpTime(t.expTime);
			tp.setTid(t.tid);
			acsws.database.pojos.Position pp = new acsws.database.pojos.Position();
			pp.setAz(t.coordinates.az);
			pp.setEl(t.coordinates.el);
			tp.setCoordinates(pp);
			targetsP.add(tp);
		}
		acsws.database.pojos.Proposal propp = new acsws.database.pojos.Proposal();
	...
	public Proposal[] getProposals() {
		...
		for (acsws.database.pojos.Proposal pp: dao.getNonObservedProposals()) {
			Proposal p = new Proposal();
			p.pid = pp.getPid();
			p.status = pp.getStatus();
			ArrayList<Target> targets = new ArrayList<Target>();
			for (acsws.database.pojos.Target tp: pp.getTargets()) {
				Target t = new Target();
				t.tid = tp.getTid();
				t.expTime = tp.getTid();
	...
	public void setProposalStatus(int pid, int status)
		...
		acsws.database.pojos.Proposal p = dao.getProposal(pid);
		p.setStatus(status);
